...
import com.webank.fabric.front.commons.pojo.performance.Data;
import com.webank.fabric.front.commons.pojo.performance.LineDataList;
import com.webank.fabric.front.commons.pojo.performance.Performance;
...
public class PerformanceService {
    ...
    public boolean toggleSync(boolean toggle) throws Exception {
        ...
        if (constants.isMonitorEnabled() == toggle) {
            ...
            throw new FrontException(ConstantCode.PERFORMANCE_EXCEPTION.getCode(), "Fail to toggle sync performance status to " + toggle);
        }
    }
    ...
    public List transferListByGap(List arrayList, int gap) {
        if (gap == 0) {
            throw new FrontException(ConstantCode.SYSTEM_EXCEPTION.getCode(), "gap cannot be 0");
        }
